...
class CryptoProject(object):
    ...
    def __init__(self):
        ...
        self.student_id = 'ytariq3'  # for test_crypto_proj_1.py
    ...
    def modular(self, local1, local2):
        c, d, _ = self.divisor(local1, local2)
        if c == 1:
            return d % local2

    def divisor(self, num1, num2):
    # A great logic to understand d https://www.youtube.com/watch?v=plxMklEvlCU
    # Source https://stackoverflow.com/questions/18940194/using-extended-euclidean-algorithm-to-create-rsa-private-key
        if num1 == 0:
            return (num2, 0, 1)
        else:
            q, s, t = self.divisor(num2 % num1, num1)
            return (q, t - (num2 // num1) * s, s)
    ...
    def crack_password_hash(self, password_hash, weak_password_list):
        ...
        for passwords in weak_password_list:
            for salts in weak_password_list:
                hashed_password = hashlib.sha256(passwords.encode() + salts.encode()).hexdigest()
    ...
    def get_factors(self, n):
        ...
        p = 0
        # one number is always ahead of sqrt(n) cause when u take sqrt() its always between its factors
        # , since p and q have the same bit size so
        # they are close to each other and we can assume that n = pq could be n = p^2
        # and p will be sqrt(n)
    ...
    def recover_msg(self, N1, N2, N3, C1, C2, C3):
        ...
        # https://www.youtube.com/watch?v=zIFehsBHB8o # good video on CRT
        # https://www.youtube.com/watch?v=f2o-DsjhKhw&t=2 # understanding modular
        # https://www.youtube.com/watch?v=nrgGU2mUum4 # using the same exponent e = 3 u can decrypt this message
        N = N1*N2*N3
        n1 = N//N1
        n2 = N//N2
        n3 = N//N3
        n1_inverse = self.modular(n1, N1)
        n2_inverse = self.modular(n2, N2)
        n3_inverse = self.modular(n3, N3)
        X = (n1*C1*n1_inverse + n2*C2*n2_inverse + n3*C3*n3_inverse)%N
        m = self.cubicRoot(X)
        return m
